...
### 5. 초기 데이터 생성 (중요!)
Git에서 다운로드 받은 직후에는 데이터 파일(`*.csv`)이 없습니다. 아래 명령어로 초기 데이터를 생성해야 합니다.
```bash
python run_analysis.py
```
> 이 명령어를 실행하면 `daily_prices.csv`, `fundamentals.csv` 등이 자동으로 생성됩니다.
...
### 3. 시스템 검증 (테스트)
시스템이 정상적으로 작동하는지 확인하려면 아래 명령어를 실행하세요.
```bash
# 전체 시스템 검증
